...
class RefundRepository extends Repository
{
    ...
    public function create(array $data)
    {
        ...
        try {
            ...
            $refund = parent::create([
                    ...
                    'shipping_amount' => $data['refund']['shipping'],
    ...
    public function getOrderItemsRefundSummary($data, $orderId)
    {
        ...
        foreach ($data as $orderItemId => $qty) {
            ...
            $summary['subtotal']['price'] += $orderItem->price * $qty;

            $summary['discount']['price'] += ($orderItem->discount_amount / $orderItem->qty_ordered) * $qty;

            $summary['tax']['price'] += ($orderItem->tax_amount / $orderItem->qty_ordered) * $qty;
        }

        $summary['shipping']['price'] += $order->shipping_invoiced - $order->shipping_refunded;
