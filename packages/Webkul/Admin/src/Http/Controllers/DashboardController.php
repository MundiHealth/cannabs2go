...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $statistics = [
            ...
            'total_sales' =>  [
                'previous' => $previous = $this->previousOrders()->sum('grand_total') - $this->previousOrders()->sum('grand_total_refunded'),
                'current' => $current = $this->currentOrders()->sum('grand_total') - $this->currentOrders()->sum('grand_total_refunded'),
                'progress' => $this->getPercentageChange($previous, $current)
            ],
            'avg_sales' =>  [
                'previous' => $previous = $this->previousOrders()->avg('grand_total') - $this->previousOrders()->avg('grand_total_refunded'),
                'current' => $current = $this->currentOrders()->avg('grand_total') - $this->currentOrders()->avg('grand_total_refunded'),
                'progress' => $this->getPercentageChange($previous, $current)
            ],
        ...
        foreach (core()->getTimeInterval($this->startDate, $this->endDate) as $interval) {
            ...
            $total = $this->getOrdersBetweenDate($interval['start'], $interval['end'])->sum('grand_total') - $this->getOrdersBetweenDate($interval['start'], $interval['end'])->sum('grand_total_refunded');
    ...
    public function getCustomerWithMostSales()
    {
        return $this->orderRepository->getModel()
            ->select(DB::raw('SUM(grand_total) as total_base_grand_total'))
