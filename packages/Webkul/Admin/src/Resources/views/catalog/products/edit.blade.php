                                    ...
                                    @if (view()->exists($typeView = 'admin::catalog.products.field-types.' . $attribute->type))
                                        ...
                                        <div class="control-group {{ $attribute->type }}" @if ($attribute->type == 'multiselect') :class="[errors.has('{{ $attribute->code }}[]') ? 'has-error' : '']" @else :class="[errors.has('{{ $attribute->code }}') ? 'has-error' : '']" @endif>
                                            ...
                                            <label for="{{ $attribute->code }}" {{ $attribute->is_required ? 'class=required' : '' }}>
                                                ...
                                                @if ($attribute->type == 'price')
                                                    <span class="currency-code">({{ core()->currencySymbol('USD') }})</span>
                                                @endif
...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            tinymce.init({
                selector: 'textarea#description, textarea#short_description, textarea#details, textarea#common_questions',
                height: 200,
                width: "100%",
                plugins: 'image imagetools media link lists wordcount save fullscreen code',
                toolbar1: 'formatselect | bold italic strikethrough | numlist bullist checklist | alignleft aligncenter alignright alignjustify |  removeformat | code',
                image_advtab: true
            });
        });
    </script>
