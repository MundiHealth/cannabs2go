...
class Core
{
    ...
    *  @return string
    */
    public function currency($amount = 0, $product = false)
    {
        ...
        $moeda_base = $this->getCurrentCurrency()->code;

        if ($product && get_class($product) != "Webkul\Product\Type\Configurable"){
            if(get_class($product) != "OnNow\Algolia\Models\Product"){
                $flat = $this->productFlatRepository->findOneWhere(['product_id' => $product->id]);
            } else {
                $flat = $product;
            }

            if ($flat->moeda_base_label == "BRL") $amount = $product->price;
            $moeda_base = $flat->moeda_base_label == "BRL" ? "USD" : $moeda_base;
        }

        if ($channel->profit_margin && $channel->profit_margin > 0) {
            $margin = $channel->profit_margin / 100;
            $amount = $amount + ($amount * $margin);
        }

        return $this->formatPrice($this->convertPrice($amount, $moeda_base), $this->getCurrentCurrency()->code);
    }
