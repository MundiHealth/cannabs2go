...
class Cart {
    ...
    {
        Event::fire('checkout.cart.add.before', ['product_id' => $productId, 'data' => $data]);

        $cart = $this->getCart();

        $order_limit_items = core()->getConfigData('sales.orderSettings.order_number.order_limit_items');

        if ($order_limit_items) {
            if(($cart->items_qty + $data['quantity']) > $order_limit_items){
                session()->flash('error', 'Você excedeu o número máximo de itens no carrinho.');
                return;
            }
        }
        ...
        if (is_string($cartProducts)) {
            ...
            foreach ($cartProducts as $key => $cartProduct) {
                ...
                $flat = $this->productFlatRepository->findOneWhere(['product_id' => $productId]);
                if($flat->moeda_base_label === "BRL"){
                    $tagetCurrency = $this->currencyRepository->findOneByField('code', $flat->moeda_base_label);
                    $exchangeRate = $this->exchangeRateRepository->findOneWhere([
                        'target_currency' => $tagetCurrency->id,
                    ]);

                    $cartProduct['base_price'] = $cartProduct['base_total'] = $cartProduct['base_price'] / $exchangeRate->rate;
                }
    ...
    public function updateItems($data)
    {
        ...
        if ($order_limit_items) {
            if(array_sum($data['qty']) > $order_limit_items){
                session()->flash('error', 'Você excedeu o número máximo de itens no carrinho.');
                return;
            }
        }
        ...
        foreach ($data['qty'] as $itemId => $quantity) {
            ...
            $flat = $this->productFlatRepository->findOneWhere(['product_id' => $item->product->id]);
            $moeda_base = $flat->moeda_base_label == "BRL" ? "USD" : core()->getCurrentCurrency()->code;

            $this->cartItemRepository->update([
                    'quantity' => $quantity,
                    'total' => core()->convertPrice($item->price * $quantity, $moeda_base),
            ...
            foreach ($items as $item) {
                if ($item->product->getTypeInstance()->compareOptions($item->additional, $data['additional'])) {
                    if (isset($data['additional']['parent_id'])) {
                        if ($item->parent && $item->parent->product->getTypeInstance()->compareOptions($item->parent->additional, request()->all()))
                            return $item;
                    } else {
                        return $item;
                    }
...
//            $this->cartRepository->delete($guestCart->id);
    ...
    public function saveCustomerAddress($data)
    {
        ...
        if (isset($data['billing']['address_id']) && $data['billing']['address_id']) {
            ...
            $billingAddress['taxvat'] = $address['taxvat'];*/

            $billingAddress['first_name'] = $this->getCurrentCustomer()->user() ? $this->getCurrentCustomer()->user()->first_name : $billingAddress['first_name'];
            $billingAddress['last_name'] = $this->getCurrentCustomer()->user() ? $this->getCurrentCustomer()->user()->last_name : $billingAddress['last_name'];
            $billingAddress['email'] = $this->getCurrentCustomer()->user() ? $this->getCurrentCustomer()->user()->email : $billingAddress['email'];

            $billingAddress = array_merge($billingAddress, $address);

            if (isset($billingAddress['date_of_birth']) && $billingAddress['date_of_birth']) {
                $this->getCurrentCustomer()->user()->date_of_birth = $billingAddress['date_of_birth'];
                $this->getCurrentCustomer()->user()->update();
            }
        }

        if (!isset($billingAddress['address_id'])){
        ...
        if ($cart->haveStockableItems()) {
            ...
            if (isset($data['shipping']['address_id']) && $data['shipping']['address_id']) {
                ...
                $shippingAddress['taxvat'] = $address['taxvat'];*/

                $shippingAddress['first_name'] = $this->getCurrentCustomer()->user() ? $this->getCurrentCustomer()->user()->first_name : $shippingAddress['first_name'];
                $shippingAddress['last_name'] = $this->getCurrentCustomer()->user() ? $this->getCurrentCustomer()->user()->last_name : $shippingAddress['last_name'];
                $shippingAddress['email'] = $this->getCurrentCustomer()->user() ? $this->getCurrentCustomer()->user()->email : $shippingAddress['email'];

                $shippingAddress = array_merge($shippingAddress, $address);
            }

            if (!isset($shippingAddress['address_id'])){
    ...
    /**
     * Define o valor da taxa de envio selecionada.
     *
     * @param float $rate
     * @return void
     */
    public function setSelectedShippingRate($rate)
    {
        if (! $cart = $this->getCart())
            return false;

        $cart->selected_shipping_rate = $rate;
        $cart->save();
    ...
    public function collectTotals()
    {
        ...
        if ($cart->base_sub_total == $cart->base_discount_amount) {
            $cart->grand_total = $cart->base_grand_total = 0;
            $cart->tax_total = $cart->base_tax_total = 0;
        }
