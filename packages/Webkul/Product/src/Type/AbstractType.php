...
abstract class AbstractType
{
    ...
    */
    ...
    public function update(array $data, $id, $attribute = "id")
    {
        ...
        foreach ($product->attribute_family->custom_attributes as $attribute) {
            ...
            if ($attribute->type == 'multiselect' || $attribute->type == 'checkbox'){
                $data[$attribute->code] = is_array($data[$attribute->code] ) ? implode(",", $data[$attribute->code]) : $data[$attribute->code];
            }
    ...
    public function getProductPrices()
    {
        return [
            'regular_price' => [
                'price' => core()->convertPrice($this->product->price, $this->product->moeda_base_label == 'BRL' ? 'USD' : core()->getCurrentCurrency()->code),
                'formated_price' => core()->currency($this->product->price, $this->product)
            ],
            'final_price' => [
                'price' => core()->convertPrice($this->getMinimalPrice(), $this->product->moeda_base_label == 'BRL' ? 'USD' : core()->getCurrentCurrency()->code),
                'formated_price' => core()->currency($this->getMinimalPrice(), $this->product)
            ]
        ];
    }
    ...
    public function prepareForCart($data)
    {
        ...
        $flat = $this->productFlatRepository->findOneWhere(['product_id' => $this->product->id]);
        $moeda_base = $flat->moeda_base_label == "BRL" ? "USD" : "BRL";
        $convertedPrice = core()->convertPrice($price, $moeda_base);
