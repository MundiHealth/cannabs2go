...
class ProductFlat
{
     ...
     * @return void
     */
    public function afterProductCreatedUpdated($product, $channel = null)
    {
        $this->createFlat($product, null, $channel = null);

        if ($product->type == 'configurable') {
            foreach ($product->variants()->get() as $variant) {
                $this->createFlat($variant, $product, $channel = null);
            }
        }
    }
     ...
     * @return void
     */
    public function createFlat($product, $parentProduct = null, $channel = null)
    {

        if (is_null($product->attribute_family))
            return;
        ...
        if (isset($product['channels'])) {
            ...
            $channels[] = $channel ?? core()->getDefaultChannelCode();
        }
                ...
                foreach ($channel->locales as $locale) {
                    ...
                    foreach ($familyAttributes[$product->attribute_family->id] as $attribute) {
                        ...
                        if ($attribute->code == 'visible_individually')
                            $productFlat->{$attribute->code} = $productAttributeValue[ProductAttributeValue::$attributeTypeFields[$attribute->type]] ?? 1;
