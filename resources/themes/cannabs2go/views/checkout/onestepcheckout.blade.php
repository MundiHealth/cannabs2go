                ...
                <div class="row">
                    <div class="col-lg-8">
                        <div class="alert alert-warning">
                            ...
                            <a href="https://api.whatsapp.com/send?phone=5511998680834" target="_blank">Dúvidas sobre a prescrição? Clique aqui e fale agora mesmo com nosso Customer Care via WhatsApp.</a>
                        </div>
        ...
        Vue.component('checkout', {
            ...
            methods: {
                ...
                validateForm: function(scope) {
                    ...
                    if (scope == 'payment-form' && $('#' + scope).find('#payment_brand').val() == 'pix'){
                        this_this.placeOrder();

                        return;
                    }

                    if (scope == 'payment-form' && $('#' + scope).find('#payment_brand').val() == 'link'){
                        this_this.placeOrder();
                    ...
                    if (scope == 'address-form') {
                        cpfExists = $('#' + scope).find('input[id="billing[taxvat]"], input[id="shipping[taxvat]"], input[id="payment[taxvat]"], .cpf');
                        if (cpfExists.length > 0){
                            if (!this_this.validateCpf(cpfExists)) {
                                return;
                            }
                        }
                    }
                ...
                validateCpf: function(input) {
                    if (!CPF.validate(input.val())){
                        swal.fire(
                            'CPF Inválido!',
                            'O CPF informado (' + input.val() + ') não é um número válido. Por favor verifique!',
                            'warning'
                        ).then((result) => {
                            input.val('').attr('placeholder', 'CPF Inválido.');
                        })
